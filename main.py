...
YOUR_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "8735260666:AAEf6MI7YAgtnaahPVM-CCwLNT6pVfRdF0E")
...
LOGIN_FAIL_COOLDOWN = 30
_login_failures = {}
_range_otp_counts = {}
_processed_ids_cache = set()
_processed_ids_loaded = False
_config_cache = {"panels": None, "groups": None, "panels_time": 0, "groups_time": 0}
_CONFIG_CACHE_TTL = 5
...
def load_panels(use_cache=False):
    if use_cache:
        now = time.time()
        if _config_cache["panels"] is not None and (now - _config_cache["panels_time"]) < _CONFIG_CACHE_TTL:
            return _config_cache["panels"]
        data = load_json(PANELS_FILE, {})
        _config_cache["panels"] = data
        _config_cache["panels_time"] = now
        return data
    return load_json(PANELS_FILE, {})

def save_panels(panels):
    _config_cache["panels"] = None
    save_json(PANELS_FILE, panels)

def load_groups(use_cache=False):
    if use_cache:
        now = time.time()
        if _config_cache["groups"] is not None and (now - _config_cache["groups_time"]) < _CONFIG_CACHE_TTL:
            return _config_cache["groups"]
        data = load_json(GROUPS_FILE, {})
        _config_cache["groups"] = data
        _config_cache["groups_time"] = now
        return data
    return load_json(GROUPS_FILE, {})

def save_groups(groups):
    _config_cache["groups"] = None
...
def save_processed_ids_bulk(new_ids):
    ...
    import threading
    data = list(_processed_ids_cache)
    threading.Thread(target=save_json, args=(PROCESSED_FILE, data), daemon=True).start()
...
def is_approved_user(user_id):
    ...
    for req in pending:
        if str(req.get("user_id")) == str(user_id) and req.get("status") == "approved":
            return True
    return False


def get_user_panel(user_id):
    pending = load_pending_requests()
    for req in pending:
        if str(req.get("user_id")) == str(user_id) and req.get("status") == "approved":
            return req.get("panel_username", "")
    return ""
...
async def addivas_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.message.from_user.id)
    user_name = update.message.from_user.first_name or "User"
    pending = load_pending_requests()
    for req in pending:
        if req.get("user_id") == user_id and req.get("status") == "pending":
            await update.message.reply_text("\u23f3 You already have a pending request. Please wait for admin approval.")
            return
    context.user_data["awaiting"] = "addivas_email"
    context.user_data["addivas_user_id"] = user_id
    context.user_data["addivas_user_name"] = user_name
    await update.message.reply_text(
        "\U0001f4e7 Send your Ivasms Email\n\nYou can cancel anytime using /cancel"
    )
...
async def addgroup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    approved = is_approved_user(user_id)
    if not owner and not approved:
        await update.message.reply_text("\U0001f6ab You are not authorized.\nUse /addivas to request an account first.")
        return
    user_panel = get_user_panel(user_id) if not owner else "all"
    ...
    if len(args) > 1:
        ...
        if group_id in groups:
            if owner:
                await show_group_manage_msg(update.message, group_id)
            else:
                await update.message.reply_text(f"\u26a0\ufe0f Group {group_id} already exists.")
            return
        if not group_id.startswith("-"):
            await update.message.reply_text("\u26a0\ufe0f Group ID should start with '-'.\nExample: /addgroup -1003087662000")
            return
        panel_assign = "all" if owner else user_panel
        groups[group_id] = {"panel": panel_assign, "active": True, "buttons": []}
        save_groups(groups)
        if owner:
            await show_group_manage_msg(update.message, group_id, new=True)
        else:
            await update.message.reply_text(
                f"\u2705 Group added!\n\n"
                f"\U0001f4e2 <b>Group:</b> <code>{group_id}</code>\n"
                f"\U0001f4c1 <b>Panel:</b> {panel_assign}\n"
                f"\U0001f7e2 <b>Status:</b> Active\n\n"
                f"OTPs from your panel will be forwarded here.",
                parse_mode='HTML'
            )
    else:
        context.user_data["awaiting"] = "cmd_add_group"
        context.user_data["user_panel"] = user_panel
        context.user_data["is_owner_cmd"] = owner
        await update.message.reply_text(
            "\U0001f4e2 Send Group ID to add:\n(e.g., -1003087662000)\n\nCancel: /cancel"
        )


async def show_group_manage_msg(message_or_query, group_id, new=False):
    groups = load_groups()
    group = groups.get(group_id)
    if not group:
        if hasattr(message_or_query, 'reply_text'):
            await message_or_query.reply_text(f"\u274c Group {group_id} not found.")
        else:
            await message_or_query.edit_message_text(f"\u274c Group {group_id} not found.")
        return
    ...
    panel_display = "All Panels" if panel == "all" else ("None" if panel == "none" else panel)
    btns = group.get("buttons", [])
    btn_count = len(btns) if isinstance(btns, list) else 0
    prefix = "\u2705 Group added!\n\n" if new else ""
    text = (
        f"{prefix}\U0001f4e2 <b>Group:</b> <code>{group_id}</code>\n\n"
        f"{status_icon} <b>Status:</b> {status_text}\n"
        f"\U0001f4c1 <b>Panel:</b> {panel_display}\n"
        f"\U0001f517 <b>Buttons:</b> {btn_count}/4"
    )
    if btns:
    ...
    if group.get("active", True):
        keyboard.append([InlineKeyboardButton("\U0001f534 Deactivate", callback_data=f"grp_deactivate:{group_id}")])
    else:
        keyboard.append([InlineKeyboardButton("\U0001f7e2 Activate", callback_data=f"grp_activate:{group_id}")])
    keyboard.append([InlineKeyboardButton(f"\U0001f4c1 Change Panel [{panel_display}]", callback_data=f"grp_panel:{group_id}")])
    if btn_count < 4:
        keyboard.append([InlineKeyboardButton("\u2795 Add Button", callback_data=f"grp_addbtn:{group_id}")])
    if btn_count > 0:
        keyboard.append([InlineKeyboardButton(f"\U0001f517 Manage Buttons ({btn_count})", callback_data=f"grp_btns:{group_id}")])
    keyboard.append([InlineKeyboardButton("\U0001f5d1 Delete Group", callback_data=f"grp_delete:{group_id}")])
    rm = InlineKeyboardMarkup(keyboard)
    if hasattr(message_or_query, 'reply_text'):
        await message_or_query.reply_text(text, parse_mode='HTML', reply_markup=rm)
    else:
        await message_or_query.edit_message_text(text, parse_mode='HTML', reply_markup=rm)
...
async def groups_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    groups = load_groups()
    if not groups:
        await update.message.reply_text("\U0001f4ed No groups configured.\n\nUse /addgroup to add one.")
        return
    text = "\U0001f4e2 <b>All Groups:</b>\n"
    keyboard = []
    for gid, g in groups.items():
        status = "\U0001f7e2" if g.get("active", True) else "\U0001f534"
        panel = g.get("panel", "all")
        panel_d = "All" if panel == "all" else ("None" if panel == "none" else panel)
        btns = len(g.get("buttons", [])) if isinstance(g.get("buttons"), list) else 0
        text += f"\n{status} <code>{gid}</code> | Panel: {panel_d} | Btns: {btns}"
        cb = f"grp_back:{gid}"
        if len(cb) <= 64:
            keyboard.append([InlineKeyboardButton(f"{status} {gid}", callback_data=cb)])
    text += "\n\n\u2b06\ufe0f Tap a group to manage it."
    await update.message.reply_text(text, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))


async def getstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.message.from_user.id)
    if not is_owner(user_id):
        await update.message.reply_text("\U0001f6ab You are not authorized.")
        return
    args = (update.message.text or "").split()
    panel_name = args[1].strip() if len(args) > 1 else None
    panels = load_panels()
    if not panels:
        await update.message.reply_text("\u274c No panels configured.")
        return
    if panel_name:
        found = None
        for name in panels:
            if name.lower() == panel_name.lower():
                found = name
                break
        if not found:
            names = ", ".join(panels.keys())
            await update.message.reply_text(f"\u274c Panel '{panel_name}' not found.\nAvailable: {names}")
            return
        await send_panel_stats(update, found, context)
    else:
        if len(panels) == 1:
            await send_panel_stats(update, list(panels.keys())[0], context)
        else:
            keyboard = []
            for name in panels:
                if panels[name].get("active", True):
                    keyboard.append([InlineKeyboardButton(f"\U0001f4ca {name}", callback_data=f"getstats:{name}")])
            keyboard.append([InlineKeyboardButton("\U0001f4ca All Panels", callback_data="getstats:__all__")])
            await update.message.reply_text("Select a panel to view stats:", reply_markup=InlineKeyboardMarkup(keyboard))


async def send_panel_stats(update_or_query, panel_name, context):
    is_query = hasattr(update_or_query, 'edit_message_text')
    send_func = update_or_query.edit_message_text if is_query else update_or_query.message.reply_text

    if is_query:
        await send_func("\u23f3 Fetching SMS statistics...")
    else:
        msg = await send_func("\u23f3 Fetching SMS statistics...")

    panels = load_panels()
    panel_config = panels.get(panel_name)
    if not panel_config:
        await send_func(f"\u274c Panel '{panel_name}' not found.")
        return
    client, csrf = await get_panel_session(panel_name, panel_config)
    if not client or not csrf:
        await send_func(f"\u274c Could not login to panel '{panel_name}'.")
        return
    ...
    try:
        ...
        first_payload = {'from': from_date_str, 'to': from_date_str, '_token': csrf}
        summary_response = await client.post(sms_url_endpoint, data=first_payload, timeout=8.0)
        summary_response.raise_for_status()
        summary_html = summary_response.text
        group_ids = re.findall(r"getDetials\('(.+?)'\)", summary_html)
        if not group_ids:
            await send_func(f"\U0001f4ca No SMS data found for panel '{panel_name}' today.")
            return
        ...
        async def count_group(group_id):
            try:
                numbers_payload = {'start': from_date_str, 'end': from_date_str, 'range': group_id, '_token': csrf}
                numbers_response = await client.post(numbers_url, data=numbers_payload, timeout=5.0)
                phone_numbers = list(dict.fromkeys(_re_number.findall(numbers_response.text)))
                if not phone_numbers:
                    return
                total_sms = 0
                async def count_number_sms(phone_number):
                    try:
                        sms_payload = {'start': from_date_str, 'end': from_date_str, 'Number': phone_number, 'Range': group_id, '_token': csrf}
                        sms_response = await client.post(sms_detail_url, data=sms_payload, timeout=5.0)
                        return len(_re_sms.findall(sms_response.text))
                    except Exception:
                        return 0
                results = await asyncio.gather(*[count_number_sms(pn) for pn in phone_numbers], return_exceptions=True)
                for r in results:
                    if isinstance(r, int):
                        total_sms += r
                range_counts[group_id] = total_sms
            except Exception:
                pass

        await asyncio.gather(*[count_group(gid) for gid in group_ids], return_exceptions=True)

        if not range_counts:
            await send_func(f"\U0001f4ca No OTP data found for '{panel_name}' today.")
            return

        country_name_match_fn = lambda gid: re.match(r'([a-zA-Z\s]+)', gid)
        lines = []
        total = 0
        sorted_ranges = sorted(range_counts.items(), key=lambda x: x[1], reverse=True)
        max_range_len = max(len(r) for r, _ in sorted_ranges)
        for range_id, count in sorted_ranges:
            lines.append(f"{range_id:<{max_range_len}}  {count:>4}")
            total += count

        date_str = today.strftime('%Y-%m-%d')
        flag = "\U0001f4ca"
        header = f"{flag} <b>SMS Statistics for</b> <code>{panel_name}</code>\n\U0001f4c5 <b>Date:</b> {date_str}\n"
        table_header = f"\n<pre>{'Range':<{max_range_len}}  {'Count':>4}\n{'─' * (max_range_len + 6)}\n"
        table_body = "\n".join(lines)
        table_footer = f"\n{'─' * (max_range_len + 6)}\n{'TOTAL':<{max_range_len}}  {total:>4}\n{'Range Count':<{max_range_len}}  {len(range_counts):>4}</pre>"
        full_msg = header + table_header + table_body + table_footer
        full_msg += f"\n\nSelect a range to fetch numbers from\nstats history:"

        keyboard = []
        for range_id, count in sorted_ranges:
            safe_range = range_id.replace(":", "_")[:30]
            cb = f"stats_range:{panel_name}:{safe_range}"
            if len(cb) <= 64:
                keyboard.append([InlineKeyboardButton(f"{range_id} ({count})", callback_data=cb)])
        context.user_data[f"stats_{panel_name}"] = range_counts

        if is_query:
            await send_func(full_msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard) if keyboard else None)
        else:
            chat_id = update_or_query.message.chat_id
            await context.bot.send_message(chat_id=chat_id, text=full_msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard) if keyboard else None)
    except Exception as e:
        await send_func(f"\u274c Error fetching stats: {e}")


async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["awaiting"] = ""
    context.user_data.pop("addivas_email", None)
    context.user_data.pop("addivas_password", None)
    context.user_data.pop("addivas_user_id", None)
    context.user_data.pop("addivas_user_name", None)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_list":
        ...
        username = req.get("panel_username", "")
        email = req.get("email", "")
        ...
        save_panels(panels)
        await query.edit_message_text(
            f"\u2705 Request Approved!\n\n\U0001f464 User: {req.get('user_name', 'Unknown')} ({req.get('user_id', '')})\n\U0001f4e7 Email: {email}\n\U0001f4c1 Panel: {username}\n\nPanel has been added and activated."
        )
        try:
            await context.bot.send_message(
                chat_id=int(req["user_id"]),
                text=f"\u2705 <b>Your request has been approved!</b>\n\n\U0001f4c1 Panel: <code>{username}</code>\n\U0001f4e7 Email: {email}\n\nYour IVASMS account is now active.",
                parse_mode='HTML'
            )
        except Exception:
            pass
        ...
        save_pending_requests(pending)
        await query.edit_message_text(
            f"\u274c Request Rejected.\n\n\U0001f464 User: {req.get('user_name', 'Unknown')} ({req.get('user_id', '')})\n\U0001f4e7 Email: {req.get('email', '')}"
        )
        try:
            await context.bot.send_message(
                chat_id=int(req["user_id"]),
                text="\u274c <b>Your request has been rejected</b> by the administrator.",
                parse_mode='HTML'
            )
        except Exception:
            pass

    elif data.startswith("grp_activate:"):
        group_id = data.split(":", 1)[1]
        groups = load_groups()
        if group_id in groups:
            groups[group_id]["active"] = True
            save_groups(groups)
        await show_group_manage_msg(query, group_id)

    elif data.startswith("grp_deactivate:"):
        group_id = data.split(":", 1)[1]
        groups = load_groups()
        if group_id in groups:
            groups[group_id]["active"] = False
    ...
    elif data.startswith("grp_panel:"):
        group_id = data.split(":", 1)[1]
        panels = load_panels()
        keyboard = []
        keyboard.append([InlineKeyboardButton("\U0001f4c1 All Panels", callback_data=f"grp_setpanel:{group_id}:all")])
        for name in panels:
            cb = f"grp_setpanel:{group_id}:{name}"
            if len(cb) <= 64:
                keyboard.append([InlineKeyboardButton(f"\U0001f4c1 {name}", callback_data=cb)])
        keyboard.append([InlineKeyboardButton("\u274c None", callback_data=f"grp_setpanel:{group_id}:none")])
        keyboard.append([InlineKeyboardButton("\u2b05\ufe0f Back", callback_data=f"grp_back:{group_id}")])
        await query.edit_message_text(f"Select panel for group <code>{group_id}</code>:", parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))

    elif data.startswith("grp_setpanel:"):
        parts = data.split(":", 2)
        group_id = parts[1]
        panel_val = parts[2]
        groups = load_groups()
    ...
    elif data.startswith("grp_addbtn:"):
        group_id = data.split(":", 1)[1]
        context.user_data["awaiting"] = f"grp_add_btn:{group_id}"
        await query.edit_message_text(
            f"\u2795 Send button for group <code>{group_id}</code>\n\nFormat: <code>text | url</code>\n\nExample:\n<code>\U0001f4ac Join Chat | https://t.me/+example</code>",
            parse_mode='HTML',
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("\u2b05\ufe0f Back", callback_data=f"grp_back:{group_id}")]])
        )

    elif data.startswith("grp_btns:"):
        group_id = data.split(":", 1)[1]
        groups = load_groups()
        group = groups.get(group_id, {})
        btns = group.get("buttons", [])
        if not btns:
            await show_group_manage_msg(query, group_id)
            return
        text = f"\U0001f517 <b>Buttons for</b> <code>{group_id}</code>:"
        for i, btn in enumerate(btns):
            text += f"\n{i+1}. {btn.get('text', '')} \u2192 {btn.get('url', '')}"
        keyboard = []
        for i, btn in enumerate(btns):
            cb = f"grp_delbtn:{group_id}:{i}"
            if len(cb) <= 64:
                keyboard.append([InlineKeyboardButton(f"\U0001f5d1 Remove: {btn.get('text', '')}", callback_data=cb)])
        keyboard.append([InlineKeyboardButton("\u2b05\ufe0f Back", callback_data=f"grp_back:{group_id}")])
        await query.edit_message_text(text, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))

    elif data.startswith("grp_delbtn:"):
        parts = data.split(":", 2)
        group_id = parts[1]
        btn_idx = int(parts[2])
        groups = load_groups()
        ...
        group_id = data.split(":", 1)[1]
        keyboard = [
            [InlineKeyboardButton("\u2705 Yes, Delete", callback_data=f"grp_delyes:{group_id}"),
             InlineKeyboardButton("\u274c Cancel", callback_data=f"grp_back:{group_id}")]
        ]
        await query.edit_message_text(f"\u26a0\ufe0f Delete group <code>{group_id}</code>?", parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))

    elif data.startswith("grp_delyes:"):
        group_id = data.split(":", 1)[1]
        groups = load_groups()
        if group_id in groups:
            del groups[group_id]
            save_groups(groups)
        await query.edit_message_text(f"\u2705 Group <code>{group_id}</code> deleted.", parse_mode='HTML')

    elif data.startswith("grp_back:"):
        group_id = data.split(":", 1)[1]
        await show_group_manage_msg(query, group_id)

    elif data.startswith("getstats:"):
        panel_name = data.split(":", 1)[1]
        if panel_name == "__all__":
            panels = load_panels()
            for name in panels:
                if panels[name].get("active", True):
                    await send_panel_stats(query, name, context)
        else:
            await send_panel_stats(query, panel_name, context)
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "addivas_email":
        context.user_data["addivas_email"] = text
        context.user_data["awaiting"] = "addivas_password"
        await update.message.reply_text("\U0001f512 Now send your Ivasms Password")
        return

    elif awaiting == "addivas_password":
        context.user_data["addivas_password"] = text
        context.user_data["awaiting"] = "addivas_username"
        await update.message.reply_text("\U0001f464 Send a unique username to access it")
        return

    elif awaiting == "addivas_username":
        import uuid
        req_user_id = context.user_data.get("addivas_user_id", str(user_id))
        req_user_name = context.user_data.get("addivas_user_name", "User")
        email = context.user_data.get("addivas_email", "")
        password = context.user_data.get("addivas_password", "")
        panel_username = text.strip().replace(" ", "")[:15]
        panels = load_panels()
        if panel_username.lower() in [p.lower() for p in panels]:
            await update.message.reply_text(f"\u26a0\ufe0f Username '{panel_username}' already exists. Send another:")
            return
        ...
        context.user_data.pop("addivas_user_name", None)
        await update.message.reply_text(
            f"\U0001f4e8 <b>Your request has been sent to administrators for approval.</b>\n\nYou will be notified once it's approved or rejected.",
            parse_mode='HTML'
        )
        owners = load_owners()
        for owner_id in owners:
            try:
                keyboard = [
                    [InlineKeyboardButton("\u2705 Approve", callback_data=f"approve_req:{req_id}"),
                     InlineKeyboardButton("\u274c Reject", callback_data=f"reject_req:{req_id}")]
                ]
                await context.bot.send_message(
                    chat_id=int(owner_id),
                    text=(
                        f"\U0001f4e8 <b>New IVASMS Request</b>\n\n"
                        f"\U0001f464 User: {req_user_name} (<code>{req_user_id}</code>)\n"
                        f"\U0001f4e7 Email: <code>{email}</code>\n"
                        f"\U0001f4c1 Username: <code>{panel_username}</code>\n"
                        f"\U0001f552 Time: {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}"
                    ),
                    parse_mode='HTML',
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
    ...
    elif awaiting == "cmd_add_group":
        group_id = text
        if not group_id.startswith("-"):
            await update.message.reply_text("\u26a0\ufe0f Group ID should start with '-'. Try again:")
            return
        is_owner_cmd = context.user_data.get("is_owner_cmd", False)
        user_panel = context.user_data.get("user_panel", "all")
        groups = load_groups()
        if group_id in groups:
            context.user_data["awaiting"] = ""
            if is_owner_cmd:
            ...
            panel_assign = "all" if is_owner_cmd else user_panel
            groups[group_id] = {"panel": panel_assign, "active": True, "buttons": []}
            save_groups(groups)
            context.user_data["awaiting"] = ""
            if is_owner_cmd:
    ...
    elif awaiting and awaiting.startswith("grp_add_btn:"):
        ...
        if "|" in text:
            ...
            if len(btns) >= 4:
                context.user_data["awaiting"] = ""
                await update.message.reply_text("\u26a0\ufe0f Maximum 4 buttons allowed!")
                return
            btns.append({"text": btn_text, "url": btn_url})
            groups[group_id]["buttons"] = btns
            save_groups(groups)
            context.user_data["awaiting"] = ""
            await update.message.reply_text(f"\u2705 Button '<b>{btn_text}</b>' added!", parse_mode='HTML')
            await show_group_manage_msg(update.message, group_id)
        else:
            await update.message.reply_text("Invalid format. Use: <code>text | url</code>", parse_mode='HTML')
        return
...
async def get_panel_session(panel_name, panel_config):
    ...
    try:
        login_page_res = await new_client.get(login_url, timeout=8.0)
        soup = BeautifulSoup(login_page_res.text, 'html.parser')
        token_input = soup.find('input', {'name': '_token'})
        login_data = {'email': panel_config["username"], 'password': panel_config["password"]}
        if token_input:
            login_data['_token'] = token_input['value']
        login_res = await new_client.post(login_url, data=login_data, timeout=8.0)
        if "login" in str(login_res.url):
        ...
        print(f"\u274c Login error for panel '{panel_name}': [{err_type}] {e}")
        _login_failures[panel_name] = now
        try:
            await new_client.aclose()
        except Exception:
...
async def fetch_sms_from_panel(client, csrf_token, panel_config):
    ...
    try:
        ...
        _re_number = re.compile(r"getDetialsNumber\('([^']+)'")
        _re_sms = re.compile(r'<p[^>]*class="[^"]*mb-0[^"]*"[^>]*>(.*?)</p>', re.DOTALL)
        _re_code1 = re.compile(r'(\d{3}-\d{3})')
        _re_code2 = re.compile(r'\b(\d{4,8})\b')

        async def fetch_group(group_id):
            msgs = []
            try:
                numbers_payload = {'start': from_date_str, 'end': to_date_str, 'range': group_id, '_token': csrf_token}
                numbers_response = await client.post(numbers_url, data=numbers_payload, timeout=4.0)
                nums_html = numbers_response.text
                phone_numbers = list(dict.fromkeys(_re_number.findall(nums_html)))
                if not phone_numbers:
                    return msgs
                ...
                async def fetch_number_sms(phone_number):
                    ...
                    try:
                        ...
                        sms_response = await client.post(sms_detail_url, data=sms_payload, timeout=4.0)
                        sms_html = sms_response.text
                        sms_matches = _re_sms.findall(sms_html)
                        for raw_sms in sms_matches:
                            sms_text = re.sub(r'<[^>]+>', '', raw_sms).strip()
                            if not sms_text:
                                continue
                            msg_id = f"{phone_number}-{sms_text}"
                            ...
                            for svc, kws in SERVICE_KEYWORDS.items():
                                if any(kw in lower_sms for kw in kws):
                                    service = svc
                                    break
                            code_match = _re_code1.search(sms_text) or _re_code2.search(sms_text)
                            num_msgs.append({
                                "id": msg_id,
                                "time": now_str, "number": phone_number,
                                "country": country_name, "flag": flag, "service": service,
                                "code": code_match.group(1) if code_match else "N/A",
                                "full_sms": sms_text, "range_id": group_id
                            })
                    except Exception:
                        pass
                    return num_msgs

                results = await asyncio.gather(*[fetch_number_sms(pn) for pn in phone_numbers], return_exceptions=True)
        ...
        results = await asyncio.gather(*[fetch_group(gid) for gid in group_ids], return_exceptions=True)
        for result in results:
            if isinstance(result, list):
                all_messages.extend(result)
        elapsed = time.time() - t0
        if all_messages:
            print(f"\u23f1 {len(all_messages)} SMS / {len(group_ids)} groups in {elapsed:.1f}s")
        return all_messages
    except Exception as e:
        print(f"\u274c SMS fetch error: [{type(e).__name__}] {e}")
        return []
...
async def check_sms_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        panels = load_panels(use_cache=True)
        groups = load_groups(use_cache=True)
...
def main():
    ...
    application.add_handler(CommandHandler("addivas", addivas_command))
    application.add_handler(CommandHandler("addgroup", addgroup_command))
    application.add_handler(CommandHandler("groups", groups_command))
    application.add_handler(CommandHandler("getstats", getstats_command))
    application.add_handler(CommandHandler("viewotps", getstats_command))
